 ...
 *   https://wordai.pro/desktop-auth#id_token=...&access_token=...&email=...
 *
 * This page reads tokens from the URL hash, calls signInWithCredential,
 * then redirects to /. If it fails, shows the error message.
 *
 * Why hash instead of query params? Hash is never sent to the server (no leaks in logs).
 */
...
export default function DesktopAuthPage() {
    ...
    useEffect(() => {
        (async () => {
            // Parse tokens from URL hash: #id_token=...&access_token=...&email=...
            const hash = window.location.hash.slice(1); // remove leading '#'
            if (!hash) {
                // Not a Tauri redirect — maybe user navigated here manually
                router.replace('/');
                return;
            }
            ...
            if (!id_token) {
                setErrorMsg('Missing id_token in URL hash');
                setStatus('error');
                return;
            }

            // Clear tokens from URL immediately (security hygiene)
            ...
            try {
                ...
                setStatus('success');
                // Navigate to hub after brief delay so user sees success message
                setTimeout(() => {
                    router.replace('/');
                }, 800);
            } catch (err: any) {
                ...
                // Store in localStorage so other parts of the app can read it
        ...
        <div style={{
            ...
            {status === 'loading' && (
                <div style={{ textAlign: 'center' }}>
                    ...
                    <h2 style={{ color: '#a855f7', margin: '0 0 8px' }}>Đang đăng nhập...</h2>
            ...
            {status === 'success' && (
                <div style={{ textAlign: 'center' }}>
                    ...
                    <h2 style={{ color: '#a855f7', margin: '0 0 8px' }}>Đăng nhập thành công!</h2>
                    <p style={{ color: '#9ca3af', margin: 0 }}>Đang chuyển hướng...</p>
                </div>
            )}
            ...
            {status === 'error' && (
                <div style={{ textAlign: 'center', maxWidth: 480, padding: '0 24px' }}>
                    ...
                    <button
                        ...
                        style={{
                            marginTop: 16, padding: '10px 24px', background: '#a855f7',
